...
var ziwei = {
	...
	computeZiWei:function (y_Solar,m_Solar,d_Solar,h_Solar,g_Solar){
		...
		dS=d_Solar;
		//取得農曆時辰，排紫微命盤
		LunarDay=LunarDate.GetLunarDay(yS,mS,dS);
		y=HeavenlyStems[(LunarDate.Year - 4) % 10]+EarthlyBranches[(LunarDate.Year - 4) % 12];
		m=LunarDate.Month>0?LunarDate.Month:LunarDate.Month*(-1);
		d=LunarDate.Day;
		h=h_Solar;
		g=g_Solar;
		//年:天干地支
	...
	//CenterPart↑↓
	getLunarDay:function(){return LunarDay + " 日 " + h + " 時";},
